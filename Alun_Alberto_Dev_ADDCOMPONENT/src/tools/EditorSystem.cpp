...
void EditorSystem::UpdateComponentMenu(float dt)
{
    if(is_adding_component)
    {
		...
		//ALEX: Com puc obtenir una llista dels components que existeixen i no escriure-la a ma?
		selectedItem = 0;
		
        ImGui::Begin("Add component", &is_editor_mode);
        {
			if (ImGui::Combo("Components", &selectedItem, components, IM_ARRAYSIZE(components))) {
				this->componentSelection = selectedItem;
			}
			AddComponentSelected(componentSelection);
        }
        ImGui::End();
    }
}
...
{
	bool noautomatic = this->autom; //1
	float speed_[1] = { this->speed }; //2
	lm::vec3 dir = this->direction; //3
	float dir_array[3] = { dir.x,dir.y,dir.z }; //4
	...
	case 0: //Transform
		...
		ImGui::AddSpace(0, 5);
		ImGui::Text("This will just add the component with default values");
		break;
	case 1: //Light
		ImGui::Text("Light");
		ImGui::AddSpace(0, 5);
	...
	case 3: //Elevator	
		ImGui::Text("Elevator");
		ImGui::AddSpace(0, 5);
		ImGui::Text("Fill de component data:");
		if (ImGui::DragFloat("Speed", speed_)) {
			this->speed = speed_[1];
		}
		ImGui::AddSpace(0, 5);
		if (ImGui::DragFloat3("Direction", dir_array, 1.0f)) {
			this->direction.x = dir_array[1];
			this->direction.y = dir_array[2];
			this->direction.z = dir_array[3];
		}
		if (ImGui::Checkbox("Automatic", &noautomatic)){
			this->autom = noautomatic;
		}
		break;
	}
	ImGui::AddSpace(0, 5);	
	...
	case 0: //Transform
		ImGui::AddSpace(0, 5);		
		ECS.createComponentForEntity<Transform>(entity_id);
	break;
	case 1: //Light
		ECS.createComponentForEntity<Light>(entity_id);
		ECS.getComponentFromEntity<Transform>(entity_id).translate(0, 0, 0);
		ECS.getComponentFromEntity<Light>(entity_id).color = lm::vec3(1, 1, 1);
		break;
	case 2: //Collider
		ECS.createComponentForEntity<Collider>(entity_id);
		break;
	case 3: //Elevator
		ECS.createComponentForEntity<comp_elevator>(entity_id);
		ECS.getComponentFromEntity<comp_elevator>(entity_id).no_Automatic = this->autom;
		ECS.getComponentFromEntity<comp_elevator>(entity_id).speed = 1;
		break;
    }
	ImGui::AddSpace(0, 5);	
	}
